...
import com.shatteredpixel.reassembledpixeldungeon.items.Gold;
...
import com.shatteredpixel.reassembledpixeldungeon.ui.ItemSlot;
			...
			if (item != null) {
				...
				if (item.name() == null) {
					...
					enable(
						...
						mode == Mode.UNIDED_OR_CURSED && ((item instanceof EquipableItem || item instanceof Wand) && (!item.isIdentified() || item.cursed || (item instanceof Armor && ((Armor) item).hasGoodGlyph()))) ||
